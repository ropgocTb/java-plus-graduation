...
Каждый сервис имеет `application.yaml`, в котором указан импорт конфигурации из `config-server`, местоположение которого
определяется с помощью Eureka.
...
- содержит контракты (DTO/эндпоинты) для обмена данными между `event-service`, `request-service`,
